...
impl<'a> N1Detector<'a> {
    ...
    fn check_field(&mut self, parent_type: &str, field: &FieldConfig, path: &[String]) {
        ...
        let is_batched = resolver.is_batched() || self.registry.has_batch_resolver(resolver.resolver_name());
...
mod tests {
    ...
    fn make_field(name: &str, field_type: FieldType, resolver: Option<ResolverConfig>) -> FieldConfig {
        FieldConfig {
            name: name.to_string(),
            field_type,
            arguments: vec![],
