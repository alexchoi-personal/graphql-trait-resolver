...
pub(crate) fn value_to_field_value(value: Value) -> FieldValue<'static> {
    match value {
        Value::List(items) => {
            FieldValue::list(items.into_iter().map(value_to_field_value))
        }
...
impl FieldResolverFactory {
    ...
    pub fn create_field(self, type_ref: TypeRef) -> Result<Field, ResolverError> {
        let resolver_config = self.field_config.resolver.clone().ok_or_else(|| {
            ResolverError::Execution("No resolver configured".to_string())
        })?;
        ...
        match resolver_config {
            ...
            ResolverConfig::Call { trait_name, args } => {
                self.create_call_field(type_ref, trait_name, args, field_name, parent_type, registry)
            }
        }
    }
