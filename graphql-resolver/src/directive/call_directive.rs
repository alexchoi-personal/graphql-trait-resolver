...
pub(crate) fn parse_call_directive(directive: &ConstDirective) -> Option<CallDirective> {
    ...
    let trait_name = get_string_argument(directive, "resolver")?;
...
mod tests {
    ...
    fn test_parse_call_directive_with_parent_field_arg() {
        ...
        obj.insert(
            Name::new("userId"),
            ConstValue::String("$parent.id".to_string()),
        );
    ...
    fn test_parse_call_directive_with_arg_mapping() {
        ...
        obj.insert(
            Name::new("id"),
            ConstValue::String("$arg.userId".to_string()),
        );
    ...
    fn test_parse_call_directive_with_literal_string() {
        ...
        obj.insert(
            Name::new("name"),
            ConstValue::String("literal value".to_string()),
        );
    ...
    fn test_const_value_to_json_boolean() {
        assert_eq!(
            const_value_to_json(&ConstValue::Boolean(true)),
            serde_json::Value::Bool(true)
        );
        assert_eq!(
            const_value_to_json(&ConstValue::Boolean(false)),
    ...
    fn test_parse_literal_number_arg() {
        ...
        obj.insert(
            Name::new("count"),
            ConstValue::Number(async_graphql_value::Number::from(10i64)),
        );
