...
fn test_n1_detection_fails_without_batch_key() {
    let sdl = r#"
        ...
        type User {
            id: ID!
            posts: [Post!]! @resolver(name: "getPostsByUser")
        }
...
fn test_register_batch_resolver() {
    let sdl = r#"
        ...
        type User {
            id: ID!
            author: Author @resolver(name: "getUsersByIds") @batchKey(field: "id")
        }
...
fn test_non_list_context_resolver_allowed() {
    let sdl = r#"
        type Query {
            user: User @resolver(name: "getUser")
        }
...
async fn test_deeply_nested_resolver_data_flow() {
    ...
    let sdl = r#"
        type Query {
            organizations: [Organization!]! @resolver(name: "getOrgs")
        }

        type Organization {
            id: ID!
            name: String!
            teams: [Team!]! @resolver(name: "getTeamsByOrg") @batchKey(field: "id")
        }

        type Team {
            id: ID!
            name: String!
            members: [Member!]! @resolver(name: "getMembersByTeam") @batchKey(field: "id")
        }
