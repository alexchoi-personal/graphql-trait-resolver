...
fn generate_resolver_impl(name: &syn::Ident, resolver_name: &str) -> TokenStream {
    let expanded = quote! {
        impl ::graphql_trait_resolver::Resolver for #name {
            ...
            fn resolve<'a>(
                &'a self,
                ctx: &'a ::graphql_trait_resolver::ResolverContext,
                args: ::std::collections::HashMap<String, ::async_graphql::Value>,
            ) -> ::graphql_trait_resolver::BoxFuture<'a, ::graphql_trait_resolver::ResolverResult<::async_graphql::Value>> {
                Box::pin(self.execute(ctx, args))
            }
        }

        ::graphql_trait_resolver::inventory::submit! {
            ::graphql_trait_resolver::ResolverRegistration::new(|| {
                Box::new(#name::default()) as Box<dyn ::graphql_trait_resolver::Resolver>
            }, #resolver_name)
        }
    };
...
fn generate_batch_resolver_registration(
    ...
    let expanded = quote! {
        ::graphql_trait_resolver::inventory::submit! {
            ::graphql_trait_resolver::BatchResolverRegistration::new(|| {
                Box::new(#name::default()) as Box<dyn ::graphql_trait_resolver::ErasedBatchResolver>
            }, #resolver_name, #batch_key)
        }
    };
    TokenStream::from(expanded)
